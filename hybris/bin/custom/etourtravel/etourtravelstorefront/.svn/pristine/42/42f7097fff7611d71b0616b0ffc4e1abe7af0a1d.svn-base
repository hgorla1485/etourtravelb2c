 ...
 *
 */
public abstract class AbstractEtourtravelb2bRegisterPageController extends AbstractPageController
{
	protected static final Logger LOG = Logger.getLogger(AbstractEtourtravelb2bRegisterPageController.class);
	...
	private Validator etourtravelb2bRegistrationValidator;

	@Resource(name = "etourtravelb2bDefaultCustomerFacade")
	private CustomerFacade etourtravelb2bDefaultCustomerFacade;
	 ...
	 */
	protected String processRegisterUserRequest(final String referer, final Etourtravelb2bRegistrationForm etourtravelb2bForm,
			final BindingResult bindingResult, final Model model, final HttpServletRequest request,
			final HttpServletResponse response, final RedirectAttributes redirectModel) throws CMSItemNotFoundException
	{
		if (bindingResult.hasErrors())
		{
			model.addAttribute(etourtravelb2bForm);
		...
		data.setFirstName(etourtravelb2bForm.getFirstName());
		data.setLastName(etourtravelb2bForm.getLastName());
		data.setLogin(etourtravelb2bForm.getEmail());
		data.setPassword(etourtravelb2bForm.getPwd());
		data.setTitleCode(etourtravelb2bForm.getTitleCode());
		data.setContactNumber(etourtravelb2bForm.getContactNumber());
		data.setCurrentLocation(etourtravelb2bForm.getCurrentLocation());

		try
		{
			//getCustomerFacade().register(data);
			getEtourtravelb2bDefaultCustomerFacade().register(data);
			getAutoLoginStrategy().login(etourtravelb2bForm.getEmail().toLowerCase(), etourtravelb2bForm.getPwd(), request, response);
